...
const GuestTrackingPage = () => {
  ...
  const handleViewBooking = (bookingId) => {
    const email = requestedWith.email || sessionStorage.getItem('guestEmail');
    const phone = requestedWith.phone || sessionStorage.getItem('guestPhone');
          ...
          <div className="bg-white rounded-2xl shadow-sm p-8 border border-gray-100">
            <div className="mb-6 text-center">
              <p className="text-gray-700 mb-2">
                Mã xác thực đã được gửi đến email <strong className="text-green-600">{recipientEmail}</strong>
              </p>
              ...
              <div className="mt-3 text-xs text-gray-500 bg-gray-50 rounded p-2">
                Tra cứu bằng: {requestedWith.email && <span className="font-medium">Email: {requestedWith.email}</span>}
                {requestedWith.email && requestedWith.phone && <span> | </span>}
                {requestedWith.phone && <span className="font-medium">SĐT: {requestedWith.phone}</span>}
              </div>
            </div>
              ...
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="text-gray-600 text-sm">
                    {requestedWith.email && <span>Email: <strong>{requestedWith.email}</strong></span>}
                    {requestedWith.email && requestedWith.phone && <span> | </span>}
                    {requestedWith.phone && <span>SĐT: <strong>{requestedWith.phone}</strong></span>}
                  </p>
                </div>
                <button
